    ...
    $.fn.areYouSure = function (options) {
        var settings = $.extend(
            {
                ...
                'dirtyClass': 'dirty',
                'change': null,
                'fieldSelector': "button:not([class='win-command']),textarea,input[type='text'][class!='ays-ignore'][role!='combobox'],input[type='number'],input[type='password'],input[type='checkbox'],input[type='radio']"
            }, options);
        ...
        var checkForm = function (evt) {
            ...
            var isFieldDirty = function ($field) {
                return ((isNullOrUndefined(getValue($field)) ? 'default' : getValue($field)) != (isNullOrUndefined($field.data('ays-orig')) ? 'default' : $field.data('ays-orig')));
            };
        ...
        $(window).bind('beforeunload', function () {
            ...
            $ignoreForms = $("form").filter('.ays-ignore');

            if ($dirtyForms.length > 0 && $ignoreForms.length == 0) {
                return settings.message;
            }
            if ($ignoreForms.length > 0)
                $ignoreForms.removeClass("ays-ignore");
        });
        ...
        return {
            ReScanOriginalValue: function (parentElement) {
                $(parentElement).find(settings.fieldSelector).each(storeOrigValue);
            }
        };
    };
})(jQuery);
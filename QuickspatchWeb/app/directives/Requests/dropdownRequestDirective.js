'use strict';
app.directive('dropdownRequest', ['$q', '$http', 'requestDataService', '$timeout', function ($q, $http, requestDataService, $timeout) {
    return {
        restrict: "E",
        scope: true,
        template: function (element, attrs) {
            return '<div class="input-group">' +
                        ...
                        '<button class="btn btn-default btn-action" type="button" ng-click="Add()" ng-if="selectedId == 0"><span class="fa fa-plus"></span></button>' +
                        '<button class="btn btn-default btn-action" type="button" ng-click="Edit()" ng-if="selectedId != 0"><span class="fa fa-edit"></span></button>' +
                        '</span>' +
                    '</div>';
        },
        link: function ($scope, $element, $attrs) {
            ...
            function refresh() {
                $scope.init().then(function (data) {
                    if (kendoControl != null) {
                        ...
                        if ($attrs["bindVal"] == 'LocationFrom' && $attrs["dropId"] === 'from') {
                            ...
                            if ($scope.data != null && $scope.data != undefined) {
                                if ($scope.data.length > 0) {
                                    for (var i = 0; i < $scope.data.length; i++) {
                                        if ($scope.data[i].KeyId == $scope.$parent.data.LocationTo) {
                        ...
                        } else if ($attrs["bindVal"] == 'LocationTo' && $attrs["dropId"] === 'to') {
                            requestDataService.getDataLocationNew().then(function (data) {
                                $scope.data = data;
                                deferred.resolve($scope.data);
                            });
                            if ($scope.data != null && $scope.data != undefined) {
                                if ($scope.data.length > 0) {
                                    for (var i = 0; i < $scope.data.length; i++) {
                                        if ($scope.data[i].KeyId == $scope.$parent.data.LocationFrom) {
                                            $scope.data.splice(i, 1);
                                        }
                                        if (kendoControl != null) {
                                            kendoControl.setDataSource($scope.data);
            ...
            $scope.$root.$on('setCourierAfterAdd', function (event, data) {
                if ($attrs["dropId"] === 'courier') {
                            ...
                            $timeout(function () {
                                $scope.$apply(function () {
                                    if ($attrs["bindObj"] == 'data') {
                                        $scope.$parent.data[$attrs["bindVal"]] = data.id;
                                    } else {
                                        $scope.$parent[$attrs["bindVal"]] = data.id;
                                    }
                                });
                            });
                        }
            ...
            function initControlerWithData() {
                $scope.init().then(function () {
                    kendoControl = $("#" + $attrs["dropId"]).kendoDropDownList({
                        ...
                        change: function (e) {
                            ...
                            $timeout(function () {
                                $scope.$apply(function () {
                                    if ($attrs["bindObj"] == 'data') {
                                        $scope.$parent.data[$attrs["bindVal"]] = value;
                                    } else {
                                        $scope.$parent[$attrs["bindVal"]] = value;
                                    }
                                });
                            });
                        }
                    ...
                    if ($attrs["bindObj"] == 'data') {
                        $scope.$parent.$watch('data.' + $attrs["referVal"], function (nval, oval) {
                            if (nval != undefined) {

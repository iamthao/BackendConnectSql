'use strict';
app.directive('timeHoldingRequest', ['$q', '$http', '$timeout', function ($q, $http, $timeout) {
    return {
        restrict: "E",
        scope: true,
                    ...
                    $scope.$apply(function () {
                        if ($attrs["bindObj"] == 'HoldingRequest') {
                            if ($attrs["bindVal"] == 'StartTime') {
                                //$scope.$parent.setTimeFrom(bind);
                                $scope.$parent.HoldingRequest[$attrs["bindVal"]] = bind;
                            }
                            if ($attrs["bindVal"] == 'EndTime') {
                                $scope.$parent.HoldingRequest[$attrs["bindVal"]] = bind;
                                //$scope.$parent.setTimeTo(bind);
                            }
                        }                       
                    });
                });

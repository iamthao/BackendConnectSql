...
app.controller('trackingController', ['$rootScope', '$scope', 'uiGmapGoogleMapApi', 'common', 'masterfileService', '$interval', '$http', '$templateCache', '$timeout',
function ($rootScope, $scope, uiGmapGoogleMapApi, common, masterfileService, $interval, $http, $templateCache, $timeout) {
        $scope.controllerId = "trackingController";
        ...
        function activate() {
            //common.activateController(null, controllerId).then(function () { log(messageLanguage.showTracking); });
            $(".angular-google-map-container").css({ height: $(window).height() - 122 });
            $timeout(function() {
                $scope.$broadcast("Courier_Change", [{ KeyId: 0, DisplayName: "---All---" }]);
            });
        ...
        $scope.viewDetail = function (requestId, e) {
            $http.get('/Request/GetRequestForTracking?requestId=' + requestId).then(function(data) {
        ...
        function showDetail(dataItem) {
            ...
            $("#popupWindow").data("VisibleTracking", true);
        ...
        $scope.BindTrackingList = function () {
            if ($scope.Tracking.Datepicker != '' && $scope.Tracking.Datepicker != undefined) {
                var url = '/Tracking/GetListTrackingData';
                if ($scope.Tracking.Courier != undefined) {

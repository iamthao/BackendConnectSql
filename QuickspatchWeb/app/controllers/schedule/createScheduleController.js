'use strict';
app.controller('createScheduleController', ['$rootScope', '$scope', 'masterfileService', 'common', 'config', 'messageLanguage',
    function ($rootScope, $scope, masterfileService, common, config, messageLanguage) {
        var events = config.events;
        $scope.controllerId = "createScheduleController";
        var getLogFn = common.logger.getLogFn;
        var log = getLogFn($scope.controllerId);
        activate();

        function activate() {
            //common.activateController(null, $scope.controllerId).then(function () { log(messageLanguage.createSchedule); });
        }
        ...
        $scope.CreateMasterfileData($scope.controllerId, 'Schedule', messageLanguage.createScheduleSuccess, callBackAfterCreateSuccess, callbackError);
